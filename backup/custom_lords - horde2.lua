...
if lordTypeName then
    out("Lord type name: " .. lordTypeName);
else
    out("Lord type name not found");
end
    ...
    characterPanel, function(child)
        if child:Id() == "dy_subtype" then
            if child:GetStateText() == lordTypeName then
                ...
                out("Candidate: " .. tempGeneralCandidateButton:Id());
            end
        elseif string.match(child:Id(), "general_candidate") then
            out("Temp candidate: " .. child:Id());
...
function attachButtonToLordRecuitment()
    core:add_listener(
        ...
        function(context)
            ...
            Util.recurseThroughChildrenApplyingFunction(
                characterPanel, function(child)
                    out(child:Id() .. " : " .. child:GetStateText());
                end
            );
